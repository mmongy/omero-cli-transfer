...
def create_filepath_annotations(
    id: str,
    conn: BlitzGateway,
    simple: bool,
...
) -> Tuple[List[XMLAnnotation], List[AnnotationRef]]:
...
def create_figure_annotations(id: str) -> Tuple[XMLAnnotation, AnnotationRef]:
    ns = id
...
def create_provenance_metadata(
    ...
    plate: bool,
) -> Union[Tuple[MapAnnotation, AnnotationRef], Tuple[None, None]]:
    if not metadata:
        return None, None
...
def create_objects(folder, filelist):
    ...
    if not filelist:
        for path, subdirs, files in os.walk(folder):
            for f in files:
                img_files.append(os.path.relpath(os.path.join(path, f), folder))
    ...
    for target in targets:
        ...
        if filelist:
            folder = par_folder
        imgs, pls, anns = parse_showinf(
            res, counter_imgs, counter_pls, counter_anns, target, folder
        )
        images.extend(imgs)
...
def run_showinf(target, cli):
    cmd = ["showinf", target, "-nopix", "-omexml-only", "-no-sas", "-noflat"]
    res = cli.popen(cmd, stdout=PIPE, stderr=DEVNULL)
    std = res.communicate()
...
def parse_files_import(text, folder):
    lines = text.split("\n")
    targets = [line for line in lines if not line.startswith("#") and len(line) > 0]
    clean_targets = []
    for target in targets:
...
def parse_showinf(text, counter_imgs, counter_plates, counter_ann, target, folder):
    ome = from_xml(text)
    images = []
    plates = []
    annotations = []
    ...
    for image in ome.images:
        ...
        if len(ome.images) > 1:  # differentiating names
            ...
            img = Image(
                id=img_id_str, name=filename + " [" + image_name + "]", pixels=pix
            )
        else:
            if not (image.name and not (image.name.isspace())):
        ...
        ns = "openmicroscopy.org/cli/transfer"
        an, anref = create_xml_and_ref(id=ann_id, namespace=ns, value=xml)
...
def create_path_xml(target):
    base = ETree.Element(
        ...
        attrib={
            ...
            "xsi:schemaLocation": "https://raw.githubusercontent.com/ome/omero-cli-transfer/"
...
def create_prepare_metadata(ann_id):
    ...
    md_dict["packing_timestamp"] = date_time
    xml = create_metadata_xml(md_dict)
    xml_ann, ref = create_xml_and_ref(id=ann_id, namespace=ns, value=xml)
    return xml_ann, ref
...
def populate_roi(
    obj: RoiI, roi_obj: IObject, ome: OME, conn: BlitzGateway
) -> Union[ROIRef, None]:
...
def populate_image(
    ...
    filepath_anns, refs = create_filepath_annotations(
        img_id, conn, simple, ds=ds, proj=proj
    )
    ...
    if fset:
        for fs_image in fset.copyImages():
            ...
            if fs_img_id not in [i.id for i in ome.images]:
                populate_image(fs_image, ome, conn, hostname, metadata, simple, fset)
    return img_ref
...
def populate_project(
    ...
    for ds in obj.listChildren():
        ...
        ds_ref = populate_dataset(
            ds_obj, ome, conn, hostname, metadata, simple, proj=str(id) + "_" + name
        )
...
def populate_screen(
    obj: ScreenI, ome: OME, conn: BlitzGateway, hostname: str, metadata: List[str]
):
    id = obj.getId()
    name = obj.getName()
    desc = obj.getDescription()
...
def populate_plate(
    obj: PlateI, ome: OME, conn: BlitzGateway, hostname: str, metadata: List[str]
) -> PlateRef:
    id = obj.getId()
    name = obj.getName()
    desc = obj.getDescription()
    ...
    filepath_anns, refs = create_filepath_annotations(
        pl.id, conn, simple=False, plate_path=plate_path
    )
    for i in range(len(filepath_anns)):
        ome.structured_annotations.append(filepath_anns[i])
...
def populate_well(
    obj: WellI, ome: OME, conn: BlitzGateway, hostname: str, metadata: List[str]
) -> Well:
    id = obj.getId()
    ...
    for index in range(obj.countWellSample()):
        ...
        ws_img_ref = populate_image(ws_img, ome, conn, hostname, metadata, simple=False)
...
def add_annotation(
    obj: Union[Project, Dataset, Image, Plate, Screen, Well, ROI],
    ann: Annotation,
    ome: OME,
    conn: BlitzGateway,
):
    if ann.OMERO_TYPE == TagAnnotationI:
        ...
        kv, ref = create_kv_and_ref(
            id=ann.getId(), namespace=ann.getNs(), value=Map(ms=mmap)
        )
        ...
        comm, ref = create_comm_and_ref(id=ann.getId(), value=ann.getTextValue())
        ...
        ts, ref = create_ts_and_ref(id=ann.getId(), value=ann.getValue().isoformat())
        ...
        long, ref = create_long_and_ref(
            id=ann.getId(), namespace=ann.getNs(), value=ann.getValue()
        )
        ...
        binaryfile = BinaryFile(
            ...
            bin_data=BinData(big_endian=True, length=length, value=b64),
        )
        f, ref = create_file_ann_and_ref(
            id=ann.getId(), namespace=ann.getNs(), binary_file=binaryfile
        )
        filepath_anns, refs = create_filepath_annotations(
            f.id, conn, simple=False, filename=ann.getFile().getName()
        )
        for i in range(len(filepath_anns)):
            ome.structured_annotations.append(filepath_anns[i])
    ...
    for ann in ome.structured_annotations:
        if isinstance(ann, FileAnnotation):
            if ann.namespace != "omero.web.figure.json":
                path = get_server_path(ann.annotation_refs, ome.structured_annotations)
                id_list[ann.id] = path
    return id_list
...
def populate_tsv(
    datatype: str, ome: OME, filepath: str, path_id_dict: dict, folder: str
):
...
def populate_rocrate(
    datatype: str, ome: OME, filepath: str, path_id_dict: dict, folder
):
    ...
    if importlib.util.find_spec("rocrate"):
        ...
        raise ImportError(
            "Could not import rocrate library. Make sure to "
    ...
    for id, file in files:
        ...
        rc.add_file(
            ...
            properties={"name": img.name, "encodingFormat": format},
        )
...
def populate_figures(ome: OME, conn: BlitzGateway, filepath: str):
    ...
    results = q.projection(
        "SELECT f.id FROM FileAnnotation f" " WHERE f.ns='omero.web.figure.json'",
        params,
        conn.SERVICE_OPTS,
    )
    ...
    for fig in figure_ids:
        ...
        if has_images:
            ...
            length = len(b64)
            fpath = os.path.join(
                fig_obj.getFile().getPath(), fig_obj.getFile().getName()
            )
            binaryfile = BinaryFile(
                file_name=fpath,
            ...
            f, _ = create_file_ann_and_ref(
                id=fig_obj.getId(), namespace=fig_obj.getNs(), binary_file=binaryfile
            )
    ...
    for k, v in ids.items():
        if v.startswith("file_annotations") or v.endswith("mock_folder"):
            ...
            if top_level == "Project":
                ...
                if proj_name:
                    if (proj_name + "/" + dataset_name + "/" + image_name) not in files:
                        files.append(proj_name + "/" + dataset_name + "/" + image_name)
    return files
...
def get_annotation_vals(cols: List[str], img: Image, ome: OME) -> List[str]:
    ...
    for annref in img.annotation_refs:
        a = next(filter(lambda x: x.id == annref.id, ome.structured_annotations))
        anns.append(a)
    vals = []
    commented = False
    for col in cols:
        if col == "filename" or col == "data_type" or col == "original_omero_ids":
            continue
        if col == "comment":
            for ann in anns:
                if (
                    isinstance(ann, CommentAnnotation)
                    and int(ann.id.split(":")[-1]) > 0
                    and not commented
                ):
            ...
            hascol = False
            for ann in anns:
                if isinstance(ann, MapAnnotation) and int(ann.id.split(":")[-1]) > 0:
...
def generate_lines_ann(
    ann: FileAnnotation, ome: OME, ids: dict, cols: List[str]
) -> List[str]:
    dest = ids[ann.id]
...
def write_lines(top_level: str, ome: OME, fp: TextIO, ids: dict, folder: str):
    ...
    paths = []
    for i in ome.images:
        tmppaths = generate_lines_and_move(
            i, ome, ids, folder, top_level, lines, columns
        )
