...
SUPPORTED = ["idonly", "imageid", "datasetid", "projectid", "plateid", "screenid"]

PLATESONLY = ["plateid", "screenid"]
...
TEST_FOLDERS = ["test/data/valid_single_image/"]
...
class TestTransfer(CLITest):
    ...
    def create_image(self, sizec=4, sizez=1, sizet=1, target_name=None):
        ...
        images.append(
            self.create_test_image(100, 100, 1, 1, 1, self.client.getSession())
        )
    ...
    def test_non_existing_object(self, target_name, tmpdir):
        self.args += ["pack", getattr(self, target_name), str(tmpdir / "test.zip")]
    ...
    def test_pack(self, target_name, tmpdir):
        ...
        args = self.args + ["pack", target, "--barchive", str(tmpdir / "testba.tar")]
        if (
            target_name == "datasetid"
            or target_name == "projectid"
            or target_name == "idonly"
    ...
    def test_pack_special(self, target_name, tmpdir):
        ...
        if (
            ...
            with pytest.raises(ValueError):
                self.cli.invoke(args, strict=True)
        args = self.args + ["pack", target, "--simple", str(tmpdir / "testsimple.tar")]
    ...
    def test_pack_metadata_only(self, target_name, tmpdir):
        if (
            ...
            self.create_plate(target_name=target_name)
        target = getattr(self, target_name)
        args = self.args + ["pack", target, "--binaries", "none", str(tmpdir)]
        self.cli.invoke(args, strict=True)
    ...
    def test_unpack_folder(self, folder_name):
        ...
        if folder_name == "test/data/valid_single_image/":
            ...
            map_ann_ids = ezomero.get_map_annotation_ids(self.gw, "Image", im_ids[-1])
            ...
            for annid in map_ann_ids:
                ...
                if ann_obj.getNs() == "openmicroscopy.org/cli/transfer":
                    assert len(ann) == 8
            assert len(ezomero.get_tag_ids(self.gw, "Image", im_ids[-1])) == 1
    ...
    def test_unpack(self, package_name):
        ...
        if package_name == "test/data/valid_single_image.tar":
            ...
            assert len(ezomero.get_roi_ids(self.gw, im_ids[-1])) == 3
            assert (
                len(ezomero.get_map_annotation_ids(self.gw, "Image", im_ids[-1])) == 3
            )
            assert len(ezomero.get_tag_ids(self.gw, "Image", im_ids[-1])) == 1
            self.delete_all()
        ...
        if package_name == "test/data/valid_single_dataset.zip":
            ...
            assert len(im_ids) == 1
            assert len(ezomero.get_map_annotation_ids(self.gw, "Dataset", ds_id)) == 1
            assert len(ezomero.get_tag_ids(self.gw, "Dataset", ds_id)) == 2
            self.delete_all()
        ...
        if package_name == "test/data/valid_single_project.zip":
            ...
            assert len(im_ids) == 1
            assert len(ezomero.get_map_annotation_ids(self.gw, "Project", pjs[-1])) == 1
            assert len(ezomero.get_tag_ids(self.gw, "Project", pjs[-1])) == 0
            self.delete_all()
        ...
        if package_name == "test/data/simple_screen.zip":
            ...
            for p in scr.listChildren():
                ...
                count += 1
            assert count == 2
            assert len(ezomero.get_tag_ids(self.gw, "Screen", sc_id)) == 1
            self.delete_all()
    ...
    def test_unpack_merge(self):
        proj_args = self.args + ["unpack", "test/data/valid_single_project.zip"]
    ...
    def test_pack_unpack_multiple_images(self, packing, tmpdir):
        ...
        images = []
        for i in range(3):
            img = self.create_test_image(100, 100, 1, 1, 1, self.client.getSession())
