...
SUPPORTED = ["idonly", "imageid", "datasetid", "projectid"]
...
class TestFigure(CLITest):
    ...
    def delete_all(self):
        ...
        if fa_ids:
            self.gw.deleteObjects(
                "FileAnnotation", fa_ids, deleteChildren=True, wait=True
            )
    ...
    def test_pack_unpack_figure(self, target_name, tmpdir):
        ...
        args = self.args + ["pack", target, "--figure", str(tmpdir / "test.tar")]
        self.cli.invoke(args, strict=True)
        self.delete_all()
        args = self.args + ["unpack", "--figure", str(tmpdir / "test.tar")]
        self.cli.invoke(args, strict=True)
        fas = self.gw.getObjects("FileAnnotation")
        count = 0
        for f in fas:
        ...
        clear_img_id = int(self.imageid.split(":")[-1])
        other_img = int(
            self.create_test_image(100, 100, 1, 1, 1, self.client.getSession()).id.val
        )
